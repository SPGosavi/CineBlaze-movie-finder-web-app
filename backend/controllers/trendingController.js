...
import { fetchTmdb, formatBasicTmdbResult, fetchEnrichedDataById } from '../services/tmdbService.js'; // Import formatter
...
export const getTrendingIndian = async (req, res) => {
    ...
    try {
      ...
      ...movies.results.slice(0, 10).map(m =>
        formatBasicTmdbResult(m, 'movie')
      ),
      ...tv.results.slice(0, 10).map(t =>
        formatBasicTmdbResult(t, 'tv')
      )
